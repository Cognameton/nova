...
class ReflectionEngine:
    ...
    def _eligible_semantic_source(self, event: MemoryEvent) -> bool:
        if event.channel != "semantic":
            return False
        return event.continuity_weight >= 0.75 or "continuity-focus" in event.tags or "nova-identity" in event.tags
    ...
    def _build_autobiographical_candidate(
        ...
        if len(unique_texts) < 2 and max((event.continuity_weight for event in cluster), default=0.0) < 0.9:
            return None
        ...
        prefix = {
            ...
            "relationship-continuity": "Relationship continuity",
            "value-continuity": "Value continuity",
            "continuity-focus": "Continuity focus",
            "self-model": "Self-model",
        }.get(theme, "Autobiographical reflection")
        summary_text = f"{prefix}: {'; '.join(unique_texts[:3])}"
        ...
        return MemoryEvent(
            ...
            tags=sorted({"autobiographical", "reflection", theme, *[tag for event in cluster for tag in event.tags if tag != "turn"]}),
