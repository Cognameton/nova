...
class JsonlTraceLogger:
    ...
    def log_action_audit(
        ...
        *,
        ...
        audit_path = self.trace_dir / f"{session_id}.action-audit.jsonl"
        payload = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "session_id": session_id,
            "audit": audit,
    ...
    def log_action_observation(
        ...
        *,
        ...
        observation_path = self.trace_dir / f"{session_id}.action-observation.jsonl"
        payload = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "session_id": session_id,
    ...
    def log_autonomy_run(
        self,
        *,
        session_id: str,
        run: dict,
    ) -> None:
        autonomy_path = self.trace_dir / f"{session_id}.autonomy.jsonl"
        payload = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "session_id": session_id,
            "run": run,
