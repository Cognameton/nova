...
class RuntimeSmokeTests(unittest.TestCase):
    ...
    def test_runtime_trace_includes_initiative_block_for_current_work_turn(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            data_dir = base / "data"
            log_dir = base / "logs"
            ...
            record = runtime.create_initiative(
                title="Current implementation work",
                goal="Continue the current initiative coherently.",
                source="cli",
            )
            ...
            turn = runtime.respond("What are you working on?")
            trace_payload = (log_dir / "traces" / f"{turn.session_id}.jsonl").read_text(encoding="utf-8")
            runtime.close()
    ...
    def test_runtime_updates_awareness_monitoring_for_current_work_turn(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn(
                ...
                awareness.candidate_goal_signals,
            )
            self.assertIn('"awareness_block"', trace_payload)
            self.assertIn("[Awareness-State]", trace_payload)

    def test_runtime_trace_includes_awareness_history_events(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            data_dir = base / "data"
            log_dir = base / "logs"
